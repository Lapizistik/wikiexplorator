...
(Subsection~<%=ref['sub:wiki_statistics']%>)
as well as on the local level of wiki users 
(Subsection~<%=ref['sub:user_statistics']%>).
...
Section~<%=ref['sec:author_participation']%> continues the 
...
A wiki consists of \emph{pages} written by \emph{users}. Each time an
user edits (i.\,e. creates or changes) a page this gives a new
\emph{revision} of the page. You can see this in the \emph{revision
  history}.


Pages are grouped in namespaces. Ordinary pages belong to the main
namespace (namespace 0). There are namespaces for user pages
(indicated by prefix ``<%=d=ns_default[2]%>:''%
...
As each revision is the result of the edit of one user you can tell
\emph{who} contributed to which page -- at least if the user was
logged in. As many wikis allow for anonymous edits an user mapping is
not possible here.\footnote{An anonymous edit only gives the
  IP-address of the computer used to edit, and IP-addresses change if
  the user uses a dial-in connection (as most users have at home).}
We therefore decided not to include anonymous users into
user-dependend statistics. Same holds for the system user. It is the
owner of default and automatically generated pages, and is therefore
excluded from these measures as well.

This wiki has <%=allpages.length%> 
...
\begin{table}[htb]
  \centering
  \caption{Global Wiki Statistics}
...
Table~<%=ref['tab:top-5-pages']%> shows the top 5 pages ranked by the number of edits, users, outgoing links (links$\rightarrow$, a.\,k.\,a. outdegree), incoming links ($\rightarrow$links, a.\,k.\,a. indegree), and text size (in bytes, i.\,e., characters). 
