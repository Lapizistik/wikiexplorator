...
First, a word on notation. An \emph{edit} is a change made to a page by a user (as in, \emph{edit this page}). A \emph{revision}, too, is a change to a page made by a user, but from the global view of the wiki (as in, the \emph{revision history} of a page). Therefore, we say that the wiki has <%=allpages.length%> pages with <%=revisions.length%> altogether revisions, and there are <%=users.length%> users with altogether <%=revisions.length%> edits.
