...
<% sonia = @params[:sonia] %>
<%
# As we do not want to run TeX more than one time we predefine \ref: 
ref = {
    'sec:statistics' => "#{count = 1}",
    'sub:wiki_statistics' => "#{count}.1",
    'sub:user_statistics' => "#{count}.2",
    ...
    'fig:revisions_per_week' => "#{count += 1}",
    'fig:cumulative_revisions_per_week' => "#{count += 1}",
...
users = wiki.users.to_a
revisions = wiki.revisions(full).to_a
revisions0 = wiki.revisions(filter0).to_a


# set time raster to weekly spells
raster = wiki.timeraster(:zero => :week, :step => :week) 
...
raster[1..-2].each_cons(2) { |s,e| 
  ...
  lpw << [e, allpages.inject(0) { |s,p| s+p.links(sfilter).length } ]
  lpw0 << [e, allpages0.inject(0) { |s,p| s+p.links(sfilter0).length } ]
  tpw << [e, pages.inject(0) { |s,p| 
            (r=p.revision(sfilter)) ? s + r.size : s }]
  tpw0 << [e, pages0.inject(0) { |s,p| 
             (r=p.revision(sfilter0)) ? s + r.size : s }]
}
...
the hyperlink network and the coauthorship network. Finally, 
...
We have <%=allpages.length%> pages with 
<%=revisions0.length%>  revisions from <%=users.length%> 
users (<%=allpages0.length%> pages with 
<%=revisions.length %> 
...
\begin{table}
  ...
  \begin{tabularx}{\linewidth}{Xrrrrrr}\toprule
    ...
    <%=p_table(p_size, 'by text size') %>\\\bottomrule
  \end{tabularx}
\end{table}
...
% Revisions vs. Pages
<%
pr = allpages.collect { |p| p.revisions.length }
...
<% 
gp_time('rpw.pdf', 'Revisions per Week', 'number of revisions', 
        rpw, 'all', rpw0, 'namespace 0')
gp_time('crpw.pdf', 'Cumulative Revisions per Week', 
