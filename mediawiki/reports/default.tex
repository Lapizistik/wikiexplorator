...
<%
filter = wiki.filter.clone # clone filter
wiki.coauthorgraph(filter) { |n| [ "label=\"\"" ] }.remove_self_links.remove_lonely_nodes.to_graphviz("coauthorgraph.pdf", "neato -Tpdf", [ "outputorder=edgesfirst;\n", "node [ shape=point, style=filled, fillcolor=white ]" ] ) { |w|  "[ weight=#{w} ]" }
%>
\begin{figure}
	\centering
...
Figure~\ref{fig:coauthor_network} shows the coauthorship network in which the nodes are authors, and an edge between any two vertices represents authorship of one or more pages.
...
<%
filter = wiki.filter.clone # clone filter
page_network = wiki.pagegraph(filter) { |n| [ "label=\"\"" ] }.remove_self_links.remove_lonely_nodes.to_graphviz("pagegraph.pdf", "neato -Tpdf", [ "outputorder=edgesfirst;\n", "node [ shape=point, style=filled, fillcolor=white ]" ] ) { |w|  "[ weight=#{w} ]" }
%>
...
Figure~\ref{fig:page_network} shows the page network in which the nodes are pages, and an edge between any two vertices represents a hyperlink from one to the other page.
