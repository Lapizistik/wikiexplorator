			...
			int itemWidth, int itemHeight, String dir)
	{
		...
		else // Recursion
		{
			...
			else if (dir.equals("up"))
			{
			      ...
			      if (newVec[2].size() > 0)
				    	createHilbertLayout(newVec[2], startX + squareHeight, startY + squareHeight, itemWidth, itemHeight, "up");
			      if (newVec[3].size() > 0)
				    	createHilbertLayout(newVec[3], startX + squareHeight, startY, itemWidth, itemHeight, "right");
			}
			else if (dir.equals("down"))
			{
			      createHilbertLayout(newVec[0], startX + squareHeight, startY + squareHeight, itemWidth, itemHeight, "right");
			      if (newVec[1].size() > 0)
				    	createHilbertLayout(newVec[1], startX + squareHeight, startY, itemWidth, itemHeight, "down");
			      if (newVec[2].size() > 0)
