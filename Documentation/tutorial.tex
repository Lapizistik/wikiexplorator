...
has the drawback that it has no direct display mode (which was rather
convenient with gnuplot), you have to create some pdf, svg, png,
etc. and use an external viewer. Thankfully we can use \code{R} for this
...
\begin{typed}
\p{irb(main):260:0>} \c{f3.pagecreation_timespan}
=> Di Mär 20 18:23:15 UTC 2007..Di Jul 28 13:53:09 UTC 2009
\p{irb(main):261:0>} \c{f3.cendtime = '2008-8-1'}
=> "2008-8-1"
\p{irb(main):262:0>} \c{f3.pagecreation_timespan}
...
=> 109
\end{typed}
Initially \cmd{f3} includes the whole page creation timespan from
March 20, 2007 to July 28, 2009. We set the page creation endtime to
August 1, 2008 and ask again for the timespan, fine, worked like
expected.  Now we ask the wiki for the pages within this timespam (84)
and compare it to the number of pages until now (109). So obviously we
got 25 ($=109-84$) new pages added in the last year.
...
=> 10
\end{typed}
We first set \cmd{f3} to full timespan (revision and page creation),
set the new revision starttime by subtracting four weeks in seconds
from the endtime, controlled that it worked and asked for the number
of revisions. Not much traffic in our example wiki the last four
weeks.
